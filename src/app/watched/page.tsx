...
import { CheckCheck, ArrowLeft, Star, CheckSquare2 } from "lucide-react";
...
function WatchedContent() {
  ...
  const [filteredMovies, setFilteredMovies] = useState<TMDBMovie[]>(watched);
  const [selectMode, setSelectMode] = useState(false);
  const [selected, setSelected] = useState<Set<number>>(new Set());

  useEffect(() => { setFilteredMovies(watched); }, [watched]);
  ...
  return (
    ...
    >
      ...
      {isGlass && (
        <>
          <div className="pointer-events-none fixed left-[-20%] top-[-15%] aspect-square w-[70vw] rounded-full opacity-30" style={{ background: "radial-gradient(circle, rgba(34,211,238,0.22) 0%, transparent 70%)", filter: "blur(90px)", zIndex: 0 }} />
          <div className="pointer-events-none fixed bottom-[-20%] right-[-15%] aspect-square w-[60vw] rounded-full opacity-25" style={{ background: "radial-gradient(circle, rgba(139,92,246,0.18) 0%, transparent 70%)", filter: "blur(110px)", zIndex: 0 }} />
        </>
      )}
      ...
      >
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-5 flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            ...
            <CheckCheck className={`w-5 h-5 ${isGlass ? "text-cyan-400" : "text-brutal-cyan"}`} strokeWidth={2.5} />
            <h1 className={`font-display font-bold text-xl sm:text-2xl tracking-tight ${isGlass ? "text-white" : "text-brutal-white uppercase"}`}>
              {isGlass ? "Watched" : "WATCHED"}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`px-2 py-0.5 text-[10px] font-bold ${isGlass ? "rounded-lg" : "brutal-chip text-brutal-cyan border-brutal-cyan"}`}
              style={isGlass ? { background: "rgba(34,211,238,0.12)", border: "1px solid rgba(34,211,238,0.30)", color: "#67E8F9" } : undefined}
            >
              {watched.length} movies seen
            </span>
            {watched.length > 0 && (
              <button
      ...
      <div className="flex-1 max-w-[1600px] mx-auto w-full pt-2 pb-8 relative z-10">
        {watched.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center px-4">
            ...
            >
              <CheckCheck className={`w-12 h-12 mx-auto mb-4 ${isGlass ? "text-slate-600" : "text-brutal-dim"}`} strokeWidth={1.5} />
              <p className={`font-bold text-lg mb-2 ${isGlass ? "text-white" : "font-display text-brutal-white uppercase"}`}>
                {isGlass ? "Nothing watched yet" : "NOTHING WATCHED YET"}
              </p>
              <p className={`text-sm mb-4 ${isGlass ? "text-slate-400" : "text-brutal-muted font-mono"}`}>
                Mark movies as watched using the <CheckCheck className="w-3 h-3 inline" /> icon on any poster.
              </p>
              <Link
                href="/"
          ...
          <>
            <ListFilterBar movies={watched} onFiltered={handleFiltered} />
                    ...
                    >
                      ...
                      {!selectMode && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <CheckCheck className={`w-10 h-10 opacity-70 ${isGlass ? "text-cyan-300" : "text-brutal-cyan"}`} strokeWidth={2} />
                        </div>
                      )}
